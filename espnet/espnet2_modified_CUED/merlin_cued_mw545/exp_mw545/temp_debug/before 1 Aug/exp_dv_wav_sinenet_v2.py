...
# No longer used, since each window is too long; use sub-window methods instead
...
def make_feed_dict_y_wav_f0_tau_train(dv_y_cfg, file_list_dict, file_dir_dict, batch_speaker_list, utter_tvt, all_utt_start_frame_index=None, return_dv=False, return_y=False, return_frame_index=False, return_file_name=False):
    ...
    for speaker_idx in range(dv_y_cfg.batch_num_spk):
        ...
        for utter_idx in range(dv_y_cfg.spk_num_utter):
            ...
            lf0_norm_data = load_cmp_file(cmp_file_name, cmp_dim=cmp_dim, feat_dim_index=lf0_index)  # Extract lf0 from cmp
            ...
            for seq_idx in range(dv_y_cfg.utter_num_seq):
                n_start = start_frame_index + seq_idx * dv_y_cfg.batch_seq_shift
...
def make_feed_dict_y_wav_f0_tau_test(dv_y_cfg, file_dir_dict, speaker_id, file_name, start_frame_index, BTD_feat_remain):
    ...
    if BTD_feat_remain is None:
        ...
        y_features_no_sil = y_features[remain_sil_before_wav:remain_sil_before_wav+len_no_sil_wav]
        B_total  = int((len_no_sil_wav - dv_y_cfg.batch_seq_len) / dv_y_cfg.batch_seq_shift) + 1
        y_features_BTD = numpy.zeros((B_total, dv_y_cfg.batch_seq_len, dv_y_cfg.feat_dim))
        ...
        for b in range(B_total):
            ...
            y_features_BTD[b, :, :] = y_features_no_sil[n_start:n_end+1, :]
            nlf_features[b, 0, 0] = lf0_mid
            tau_features[b, 0, 0] = win_pitch_loc
    else:
        y_features_BTD, nlf_features, tau_features = BTD_feat_remain
        B_total = y_features_BTD.shape[0]
    ...
    if B_remain > 0:
        ...
        for b in range(B_remain):
            y_feat_remain[b]   = y_features_BTD[b + B_actual]
...
class dv_y_wav_cmp_configuration(dv_y_configuration):
    ...
    def __init__(self, cfg):
        ...
        self.nn_layer_config_list = [
            ...
            {'type':'SinenetV2', 'size':256, 'num_channels':8, 'channel_combi':'stack', 'dropout_p':0, 'batch_norm':False},
