...
from espnet2_modified_CUED.tts.spk_embed_model.spk_embed_y_model import Build_spk_embed_y_model
...
class Tacotron2(AbsTTS):
    ...
    def __init__(
        ...
        self.use_spk_model = use_spk_model
        self.spk_model_name = spk_model_name
        ...
        if use_teacher_forcing:
            ...
            if self.use_spk_model:
                if self.spk_model_name=='gst':
                    ...
                    if self.spk_model_integration_type == 'add':
                        h = h + spk_embs.unsqueeze(1)
                    elif self.spk_model_integration_type == 'concat':
                        spk_embs = spk_embs.unsqueeze(1).expand(-1, h.size(1), -1)
                        h = torch.cat([h, spk_embs], dim=-1)
