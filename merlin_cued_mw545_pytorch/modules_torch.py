...
class dv_y_model(nn.Module):
    def __init__(self, dv_y_cfg):
        super(dv_y_model, self).__init__()
    ...
    def build_train_step(self):
        # TODO: try adam, and find ways to pass in learning rate
    ...
    def gen_lambda_SD(self, feed_dict):
        self.lambda_SBD = self.gen_lambda_SBD(feed_dict)
        lambda_SD = self.torch_reduce_SBD_to_SD(self.lambda_SBD)
        return lambda_SD

    def gen_logit_SBD(self, feed_dict):
        # Same as the function forward, here for distinction between SBD and SD
        self.lambda_SBD = self.gen_lambda_SBD(feed_dict)
        logit_SBD = self.lambda_to_logit_SBD(self.lambda_SBD)
        return logit_SBD

    def gen_logit_SD(self, feed_dict):
        lambda_SD = self.gen_lambda_SD(feed_dict)
        logit_SD = self.lambda_to_logit_SD(self.lambda_SD)
        return logit_SD

    def gen_train_loss(self, feed_dict):
        # TODO: need to modify this
        # reshape output to 2D
        # target is 1D
        # change data feeding module too
        # Cannot use a dict, need to use a list
        # [y, lf0, pitch, ..., ..., target_S]
        target_S = feed_dict[-1]
        if self.dv_y_cfg.train_by_window:
            self.logit_SBD = self.gen_logit_SBD(feed_dict)
            # Get B value
            b = self.dv_y_cfg.spk_num_seq
            target_SBD = self.numpy_expand_SD_to_SBD(target_S, b)
            self.loss  = criterion(self.logit_SBD, target_SBD)
        else:
            self.logit_SD = self.gen_logit_SD(feed_dict)
            target_SD = target_SD
            self.loss = criterion(self.logit_SD, target_S)
        return self.loss

    def train_model_param(self, feed_dict):
        self.loss = self.gen_train_loss(self, feed_dict)
        self.loss.backward()
        self.optimizer.step()
    ...
    def save_current_model(self, nnets_file_name=None):
        ...
        try:
            # Use Torch saver
            torch.save(model.state_dict(), nnets_file_name)
        except:
    ...
    def load_prev_model(self, previous_model_name=None):
        ...
        try:
            self.load_state_dict(torch.load(previous_model_name))
            # logger.info('use TF saver')
        except:
            self.logger.info('cannot use TF saver, use cPickle')
            model = cPickle.load(open(nnets_file_name, 'rb'))

    def forward(self, feed_dict):
        self.logit_SBD = self.gen_logit_SBD(feed_dict)
        return self.logit_SBD

    def update_learning_rate(self, learning_rate):
        self.learning_rate = learning_rate

    def numpy_expand_SD_to_SBD(self, v_SD, b=0):
        if b <= 0:
            b = self.dv_y_cfg.spk_num_seq
        # Expand use numpy
        v_S1D = numpy.expand_dims(v_SD, axis=1)
        v_SBD = numpy.tile(v_S1D, (1,b,1))
        return v_SBD

    def torch_reduce_SBD_to_SD(self, v_SBD):
        v_SD = torch.mean(v_SBD, dim=1, keepdim=False)
        return v_SD

    def lambda_to_logit_SBD(self, lambda_SBD):
        logit_SBD = self.classify_layer(lambda_SBD)
        return logit_SBD

    def lambda_to_logit_SD(self, lambda_SD):
        logit_SD = self.classify_layer(lambda_SD)
        return logit_SD
...
class dv_y_cmp_model(dv_y_model):
    ...
    def build_layers(self):
        ...
        # Input Layer
        self.input_layer = build_SBTD_input_layer(dv_y_cfg)
        prev_layer = self.input_layer
        # Hidden Layers
        for i in range(dv_y_cfg.num_nn_layers):
            new_layer = build_nn_layer(dv_y_cfg.nn_layer_config_list[i], prev_layer, self)
            # Setting up for the next layer
            prev_layer = new_layer
            self.nn_layers.append(new_layer)
        # Output Layer
        self.output_layer = build_SBD_output_layer(dv_y_cfg, prev_layer)

        # Classify layer maps lambda to logits
        self.classify_layer = self.output_layer

    def gen_lambda_SBD(self, feed_dict):
        tensor_list = self.input_layer(feed_dict)
        for i in range(dv_y_cfg.num_nn_layers):
            tensor_list = self.nn_layers[i](tensor_list)
        return tensor_list['h']

class build_SBTD_input_layer(nn.Module):
    def __init__(self, dv_y_cfg):
        super(build_SBTD_input_layer, self).__init__()
        self.params = {}
...
class build_nn_layer(nn.Module):
    def __init__(self, layer_config, prev_layer, model):
        super(build_nn_layer, self).__init__()
        self.params = {}
        ...
        construct_layer = getattr(self, self.params["type"])
        construct_layer()

        self.build_dropout()

    def build_dropout(self, dropout_input):
        ...
        if self.params["dropout_p"] > 0:
            self.dropout_layer = nn.Dropout(p=self.params["dropout_p"])

    def forward_dropout(self, h):
        if self.params["dropout_p"] > 0:
    ...
    def forward(self, tensor_inputs):
        # Tensor reshape h first
        h_input_reshape, self.params["expect_input_dim_values"] = tensor_reshape(tensor_inputs[0], self.params["input_dim_seq"], self.params["input_dim_values"], self.params["expect_input_dim_seq"])
        tensor_outputs = self.forward_layer(h_input_reshape, tensor_inputs)
        # Apply dropout in place
        tensor_outputs[0] = self.forward_dropout(tensor_outputs[0])
    ...
    def ReLUDVMax(self, layer_config=None):
        ...
        def forward_layer(self, h_input_reshape, tensor_inputs):
